...
__global__ void d_STCNaive(ouroGraph<VertexDataType, EdgeDataType, MemoryManagerType>* graph,
	                       uint32_t* triangles)
{
	...
	for(auto i = 0U; i < (vertex.meta_data.neighbours - 1); ++i)
	{
		...
		for(auto j = i + 1; j < vertex.meta_data.neighbours; ++j)
		{
			...
			auto running_vertex = graph->vertices.getAt(running_index);
			while(true)
			{
...
//
template <typename VertexDataType, typename EdgeDataType, typename MemoryManagerType>
__global__ void d_STCNaive_w(ouroGraph<VertexDataType, EdgeDataType, MemoryManagerType>* graph,
                             uint32_t* triangles)
{
...
//
template <typename VertexDataType, typename EdgeDataType, typename MemoryManagerType>
__global__ void d_STCBalanced(ouroGraph<VertexDataType, EdgeDataType, MemoryManagerType>* graph,
                              uint32_t* triangles,
                              ...
                              int warp_count)
{
	...
	auto vertex = graph->vertices.getAt(index);
...
//
template <typename VertexDataType, typename EdgeDataType, typename MemoryManagerType>
__global__ void d_STCBalanced_w(ouroGraph<VertexDataType, EdgeDataType, MemoryManagerType>* graph,
                                uint32_t* triangles,
                                vertex_t* vertex_index,
                                vertex_t* page_per_vertex_index,
                                int warp_count)
                                             ...
                                             vertex_t* d_page_count)
{
	...
	d_page_count[tid] = Ouro::divup(vertex.meta_data.neighbours, step_size);
}

// ##############################################################################################################################################
//
template <typename VertexDataType, typename EdgeDataType, typename MemoryManagerType>
__global__ void d_calculateOffsetsForSTC(ouroGraph<VertexDataType, EdgeDataType, MemoryManagerType>* graph,
...
template <typename VertexDataType, typename EdgeDataType, typename MemoryManagerType>
uint32_t STC<VertexDataType, EdgeDataType, MemoryManagerType>::algSTCNaive(const ouroGraph<VertexDataType, EdgeDataType, MemoryManagerType>& graph, bool return_global_TC)
{
	uint32_t triangle_count;

	d_triangles.memSet(0, graph.number_vertices);
	...
	if (return_global_TC)
	{
		...
		thrust::inclusive_scan(th_triangles, th_triangles + graph.number_vertices, th_triangle_count);

		// Copy result back to host
		d_triangles.copyFromDevice(triangles.get(), graph.number_vertices);
		d_triangle_count.copyFromDevice(&triangle_count, 1, (graph.number_vertices - 1));
...
template <typename VertexDataType, typename EdgeDataType, typename MemoryManagerType>
uint32_t STC<VertexDataType, EdgeDataType, MemoryManagerType>::algSTCBalanced(const ouroGraph<VertexDataType, EdgeDataType, MemoryManagerType>& graph, bool return_global_TC)
{
	uint32_t triangle_count;
