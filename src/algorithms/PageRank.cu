...
__global__ void d_PageRankNaive(DynGraph<VertexDataType, EdgeDataType, MemoryManagerType>* graph,
                                float* page_rank,
...
//
template <typename VertexDataType, typename EdgeDataType, typename MemoryManagerType>
__global__ void d_PageRankNaive_w(DynGraph<VertexDataType, EdgeDataType, MemoryManagerType>* graph,
                                  float* page_rank,
                                  float* next_page_rank)
{
...
//
template <typename VertexDataType, typename EdgeDataType, typename MemoryManagerType>
__global__ void d_PageRankBalanced(DynGraph<VertexDataType, EdgeDataType, MemoryManagerType>* graph,
                                   float* page_rank,
                                   float* next_page_rank,
                                   ...
                                   int warp_count)
{
	...
	VertexDataType vertex = graph->d_vertices[index];
...
//
template <typename VertexDataType, typename EdgeDataType, typename MemoryManagerType>
__global__ void d_PageRankBalanced_w(DynGraph<VertexDataType, EdgeDataType, MemoryManagerType>* graph,
                                     float* page_rank,
                                     float* next_page_rank,
                                     vertex_t* vertex_index,
                                     vertex_t* page_per_vertex_index,
                                             ...
                                             vertex_t* d_page_count)
{
	...
	// PageRank
	VertexDataType vertex = graph->d_vertices[tid];
	d_page_count[tid] = divup(vertex.meta_data.neighbours, WARP_SIZE);
}

// ##############################################################################################################################################
//
template <typename VertexDataType, typename EdgeDataType, typename MemoryManagerType>
__global__ void d_calculateOffsetsForPageRank(DynGraph<VertexDataType, EdgeDataType, MemoryManagerType>* graph,
...
//
template <typename VertexDataType, typename EdgeDataType, typename MemoryManagerType>
__global__ void d_applyPageRank(DynGraph<VertexDataType, EdgeDataType, MemoryManagerType>* graph,
                                float* page_rank,
                                float* next_page_rank,
...
template <typename VertexDataType, typename EdgeDataType, typename MemoryManagerType>
float PageRank<VertexDataType, EdgeDataType, MemoryManagerType>::algPageRankNaive(const DynGraph<VertexDataType, EdgeDataType, MemoryManagerType>& dyn_graph, PerfMeasure& performance)
{
	float absDiff = 0.0f;
	...
	grid_size = divup(dyn_graph.number_vertices, block_size);
	...
	thrust::inclusive_scan(th_abs_diff, th_abs_diff + dyn_graph.number_vertices, th_diff_sum);

	d_diff_sum.copyFromDevice(&absDiff, 1, (dyn_graph.number_vertices - 1));
...
template <typename VertexDataType, typename EdgeDataType, typename MemoryManagerType>
float PageRank<VertexDataType, EdgeDataType, MemoryManagerType>::algPageRankBalanced(const DynGraph<VertexDataType, EdgeDataType, MemoryManagerType>& dyn_graph, PerfMeasure& performance)
{
	float absDiff = 0.0f;
	start_clock(ce_start, ce_stop);
...
// Instantiations
template float PageRank<VertexData, EdgeData, OuroCQ>::algPageRankNaive(const DynGraph<VertexData, EdgeData, OuroCQ>&, PerfMeasure&);
template float PageRank<VertexData, EdgeData, OuroPQ>::algPageRankNaive(const DynGraph<VertexData, EdgeData, OuroPQ>&, PerfMeasure&);
template float PageRank<VertexData, EdgeData, OuroCQ>::algPageRankBalanced(const DynGraph<VertexData, EdgeData, OuroCQ>&, PerfMeasure&);
template float PageRank<VertexData, EdgeData, OuroPQ>::algPageRankBalanced(const DynGraph<VertexData, EdgeData, OuroPQ>&, PerfMeasure&);